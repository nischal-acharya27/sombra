...
**Dialogue.** Duryodhana carries a full intro/defeat scene — 13 intro beats and 8 defeat beats, matching the Warden treatment Shakuni and Bakasura already got (`docs/DECISIONS.md` § "A Warden's intro and defeat are a scene, not a boundary"), extended here to the act's boss on the same reasoning: he is the roster's most human, nuanced antagonist, and a bare two-line minimum undersold that. His enrage crossing still carries one authored phase-transition line, not the ordinary generic warning — his "no phase-transition" flag is about the visual reveal, not speech, and the enrage threshold remains the natural hook for a line about his own grievance. Drafted in `strings.js` as `GATE3_DURYODHANA_INTRO_1..13`, `GATE3_DURYODHANA_ENRAGE`, and `GATE3_DURYODHANA_DEFEAT_1..8`, alongside (not replacing) the currently-live old-spec `GATE3_GORUMUKH_*` keys, since Duryodhana has not yet been wired as a `Boss` subclass or gate file.
